...
!  * Jacob Williams, converted to Fortran 4/30/2020
    ...
    type,public :: State
        ...
        real(wp) :: dynamic_viscosity = 0_wp
        contains
        procedure :: density
    ...
    public :: COESA_density,COESA_atmosphere
...
pure real(wp) function mean_molecular_weight_ratio_lower(Z)
    ...
    !     error "altitude above maximum value in table"
    else
        i = find(Z, Ztable)
...
pure real(wp) function dynamic_viscosity(Z,T)
    ...
    ! if (Z > 86000.0_wp) error  'unable to compute dynamic viscosity above 86km'
...
function COESA_atmosphere(Z) result(s)
    ...
    type(state) :: s
    real(wp) :: H,M,T,P,c,dynam_vis
    
    if (Z < 86000.0_wp) then
        H = geopotential_altitude(Z)
        M = mean_molecular_weight_lower(Z)
        T = temperature_lower(H, M)
        P = pressure_lower(H)
        c = speed_of_sound_lower(T, M)
        dynam_vis= dynamic_viscosity(Z,T)
    else
    ...
    s = State(M, T, P, c,dynam_vis)
end function COESA_atmosphere

function COESA_density(Z,density,P,T) 
    implicit none
    !! a simple version that just returns density
    real(wp),intent(in) :: Z !! altitude in meters
    real(wp),intent(out) :: density,P,T !! density,pressure,a nd temprature
    integer :: COESA_density 
    real(wp) :: H,M
