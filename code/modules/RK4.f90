...
subroutine r8_rkf45 ( f, neqn, y, yp, t, tout, relerr, abserr, flag )
  ...
  !
    ...
    real ( kind = 8 ) abserr
    real ( kind = 8 ) :: abserr_save = -1.0D+00
    real ( kind = 8 ) ae
    real ( kind = 8 ) dt
    real ( kind = 8 ) ee
    real ( kind = 8 ) eeoet
    ...
    real ( kind = 8 ) f4(neqn)
    real ( kind = 8 ) f5(neqn)
    real ( kind = 8 ) :: h = -1.0D+00
    logical hfaild
    real ( kind = 8 ) hmin
    integer ( kind = 4 ) flag
    integer ( kind = 4 ) :: flag_save = -1000
    integer ( kind = 4 ) :: init = -1000
    integer ( kind = 4 ) k
    integer ( kind = 4 ) :: kflag = -1000
    integer ( kind = 4 ) :: kop = -1
    integer ( kind = 4 ), parameter :: maxnfe = 3000
    integer ( kind = 4 ) mflag
    integer ( kind = 4 ) :: nfe = -1
    logical output
    real ( kind = 8 ) relerr
    real ( kind = 8 ) relerr_min
    real ( kind = 8 ) :: relerr_save = -1.0D+00
